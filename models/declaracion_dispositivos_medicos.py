...
class DeclaracionDispositivosMedicos(models.Model):
    _name = 'arsante.declaracion_dispositivos_medicos'
    ...
    def _default_get(self):
        tipo_id = self.env['arsante.tipo_registro'].search([('tipo', '=', 'declaracion_dispositivos_medicos')])
        return tipo_id
    ...
    def open_pdf_resolucion_url(self):
        """Abre el link de acceso a la carpeta del PDF"""
        self.ensure_one()
    ...
    def create_so(self):
        ...
        ids = self.env["arsante.declaracion_dispositivos_medicos"].browse(self._context.get("active_ids", []))
