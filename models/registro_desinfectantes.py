...
class CdaCosmetico(models.Model):
    _name = 'arsante.registro_desinfectantes'

    def _default_get(self):
        tipo_id=self.env['arsante.tipo_registro'].search([('tipo','=','registro_desinfectantes')])
        return tipo_id
    ...
    sale_order_id = fields.Many2one(comodel_name='sale.order', string='Notas de Venta')
    ...
    def create_so(self):
        ...
        ids = self.env["arsante.registro_desinfectantes"].browse(self._context.get("active_ids", []))
        ...
        for i in ids:
            if i.nro_registro and i.nro_registro and i.product_id:
                ...
                Value={
                    'name':'Nro. Registro: '+i.nro_registro+' Nro. Resolución: '+i.nro_resolucion,
