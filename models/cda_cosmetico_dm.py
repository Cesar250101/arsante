...
class CdaCosmetico(models.Model):
    _name = 'arsante.cda_cosmetico_dm'

    def _default_get(self):
        tipo_id=self.env['arsante.tipo_registro'].search([('tipo','=','cda_cosmetico')])
        return tipo_id
    ...
    def create_so(self):
        ...
        ids = self.env["arsante.cda_cosmetico_dm"].browse(self._context.get("active_ids", []))
        ...
        for i in ids:
            if i.au and i.nro_cda and i.proveedor_id and i.marca:
                ...
                raise Warning("""A algunos registros les falta uno de los siguierntes datos:
                              ...
                              """)
        # sale_order_id.write({
        #     'order_lines':[(6, 0, [sale_order_line_ids])]
        # })
    ...
    def create(self, vals):
        ...
        # tipo_registro_id=self.env['arsante.tipo_registro'].search([('id','=',self.tipo_registro_id.id)])
        # tipo_registro_id.write({
        #     'cda_cosmetico_ids':[(4, 0, [self.id])]
        # })
