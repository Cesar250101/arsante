...
class CdaCosmetico(models.Model):
    _name = 'arsante.renovaciones_cosmeticos'

    def _default_get(self):
        tipo_id=self.env['arsante.tipo_registro'].search([('tipo','=','renovaciones_cosmeticas')])
        return tipo_id
    ...
    clave_gicona = fields.Char(string='Referencia Gicona')
    product_id = fields.Many2one(comodel_name='product.product', string='Nombre Producto')
    ...
    def create_so(self):
        ...
        for i in ids:
            if i.clave_gicona and i.nro_registro and i.product_id and i.nro_resolucion:
                ...
                Value={
                    'name':'Referencia Gicona: '+i.clave_gicona+' Nº Registro: '+i.nro_registro+' Nro. Resolución: '+i.nro_resolucion,
