...
class EximCosmeticos(models.Model):
    _name = 'arsante.exim_proceso_cosmeticos'

    def _default_get(self):
        tipo_id=self.env['arsante.tipo_registro'].search([('tipo','=','exim_proceso_cosmeticos')])
        return tipo_id
    ...
    enviar_cliente = fields.Boolean(string='Enviar Cliente')
    ...
    @api.depends('product_id','fabricante_id')
    def _compute_name(self):
        for i in self:
            if i.product_id and i.fabricante_id:
                i.name=i.product_id.name+' ; '+i.fabricante_id.name
